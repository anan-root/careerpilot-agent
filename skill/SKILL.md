...
- 城市：上海
- 岗位类型：社招/全职
- 平台：默认 BOSS直聘、智联招聘、前程无忧；可选猎聘、拉勾、牛客网、应届生、国聘网、丁香人才网、就业在线
- 大模型：DeepSeek，OpenAI SDK 兼容调用方式
- 安全策略：默认不自动打开浏览器，不自动打开 Boss 登录页，不绕过登录、验证码或滑块；目标里明确写“允许 Boss 登录浏览器”时才会尝试
...
- 默认上海、社招、BOSS直聘/智联招聘/前程无忧。
- 默认看社招和校招，排除实习；用户也可以明确只看某一类。
- `双休优先` 和根据毕业时间推断的经验偏好默认只用于排序与风险提示；明确写出“只看双休”或经验上限时才硬筛选。
