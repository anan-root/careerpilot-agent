...
这些字段取决于平台是否在列表页公开。部分平台把福利、详细地址、完整经验要求放在详情页，详情页又可能需要登录或页面确认。CareerPilot Agent 会尝试记录字段完整度，但不会为了填字段而编造信息。

### 为什么 Boss 登录态会失效？

BOSS 的登录状态不是项目自己签发的。平台可能根据会话有效期、设备环境、访问频率或页面确认要求重新登录，所以同一个浏览器配置也不能保证长期稳定在线。需要 BOSS 登录浏览器时请手动开启，并在失效后按平台页面重新完成登录。现在浏览器路径会先做软判断、再重试一次，状态不稳时也会尽量保留窗口，避免一误判就退出。
...
- 自然语言求职任务配置：任务解析、保存、加载、修改、删除、按任务检索。
- Streamlit 工作区拆分：任务配置、岗位结果、沟通行动、记录记忆。
- BOSS 受控沟通：草稿生成、自定义字数和提示词边界、干跑检查、确认发送、本地记录。
...
- 统一岗位 Schema、手动 JD/链接导入、本地 API 和浏览器插件雏形。
- 独立 Vite + Vue 前端骨架：岗位采集、导入、简历匹配和岗位动作。

下一步建议：

- 更强的 Agent 对话：基于当前报告和岗位列表进行多轮解释。
- 面试准备工作台：题库沉淀、模拟问答、复盘计划。
- 更强的详情页解析：针对 BOSS、智联、前程无忧等页面做平台级字段适配。
